...
import com.luisfcanals.deriva.future.Future;
import com.luisfcanals.deriva.market.Market;
import com.luisfcanals.deriva.bond.TreasureBond;
...
public class TreasureFuture extends Future {
    ...
    public double getPrice(final Calendar date) {
        if(date.before(this.maturityTime)) {
            final double continuousRf = Math.log(1 + this.riskFreeRate);
            final double price1 =
0;//                    this.bond.getPrice(date) * Math.exp(continuousRf * T);
